...
def use_controlled_hadamard(initial_state: Tuple[List[int], int] = None, do_draw: bool = False) -> QuantumCircuit:
    qc_direct = QuantumCircuit(2)
    if initial_state:
        qc_direct.initialize(initial_state[0], initial_state[1])
    qc_direct.ch(control_qubit, target_qubit)
...
def constructed_controlled_hadamard(initial_state: Tuple[List[int], int] = None, do_draw: bool = False) -> QuantumCircuit:
    ...
    angle = pi/4
    qc_indirect.ry(angle, target_qubit)
...
for initial_state in initial_states:
    use_controlled_hadamard(initial_state)
